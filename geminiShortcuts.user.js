...
(function () {
    ...
    const CFG = {
        ...
        selectors: {
            ...
            messageTurnFallback: '.user-query-container, .query-content, .query-text',
    ...
    function scrollToMessageTurn(direction) {
        ...
        const threshold = 80;
        const turnInfos = turns.map((turn) => {
            let el = turn;
            let rect = el.getBoundingClientRect();
            if ((!rect.height && !rect.width) && CFG.selectors.messageTurnFallback) {
                const fallback = turn.querySelector(CFG.selectors.messageTurnFallback);
                if (fallback) {
                    el = fallback;
        ...
        let currentIndex = turnInfos.findIndex(({ rect }) => rect.top <= threshold && rect.bottom >= threshold);
        if (currentIndex === -1) {
            for (let i = turnInfos.length - 1; i >= 0; i--) {
                if (turnInfos[i].rect.top < threshold) {
                    currentIndex = i;
                    break;
                }
            }
